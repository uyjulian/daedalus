	...
	if( current_origin != last_origin )
	{
		...
		if(!gFrameskipActive)
		{
			if( gGlobalPreferences.DisplayFramerate )
			{
				...
				{
					case 1:
						sprintf(str_buf, "%#.1f  ", gCurrentFramerate );
						break;
					case 2:
						sprintf(str_buf, "FPS[%#.1f] VB[%d/%d] Sync[%#.1f%%]   ", gCurrentFramerate, u32( Fsync * f32( FramerateLimiter_GetTvFrequencyHz() ) ), FramerateLimiter_GetTvFrequencyHz(), Fsync * 100.0f );
						break;
					case 3:
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
						sprintf(str_buf, "Dlist[%d] Cull[%d] | Tris[%d] Cull[%d] | Rect[%d] Clip[%d] ", gNumInstructionsExecuted, gNumDListsCulled, gRenderer->GetNumTrisRendered(), gRenderer->GetNumTrisClipped(), gRenderer->GetNumRect(), gNumRectsClipped);
#else
						sprintf(str_buf, "%#.1f  ", gCurrentFramerate );
#endif
						break;
...
void CGraphicsPluginImpl::RomClosed()
{
	...
	DBGConsole_Msg(0, "Finalising PS2Graphics");
	#endif
	DLParser_Finalise();
...
{
	#ifdef DAEDALUS_DEBUG_CONSOLE
	DBGConsole_Msg( 0, "Initialising Graphics Plugin [CPS2]" );
#endif
