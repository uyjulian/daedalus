...
extern void gsTexEnvColor(int color);
...
{	
	gsTexFunc(GS_TFX_MODULATE,GS_TCC_RGBA);	
}
...
{
	gsTexEnvColor(details.EnvColour.GetColour());
	gsTexFunc(GS_TFX_BLEND,GS_TCC_RGB);
}
...
void BlendMode_0x003432685566ff7fLL (BLEND_MODE_ARGS)
{
	...
	gsTexEnvColor(details.EnvColour.GetColour());
	gsTexFunc(GS_TFX_BLEND,GS_TCC_RGBA);
}
...
void BlendMode_0x001147fffffffe38LL (BLEND_MODE_ARGS)
{
	//details.ColourAdjuster.SetA( details.PrimColour );
	//gsTexFunc(GS_TFX_BLEND,GS_TCC_RGBA);
	gsTexFunc(GS_TFX_ADD,GS_TCC_RGBA);
}
...
{
	gsTexFunc(GS_TFX_MODULATE,GS_TCC_RGB);
}
...
{
	details.ColourAdjuster.SetA(details.PrimColour);
	gsTexEnvColor(details.PrimColour.GetColour());
	gsTexFunc(GS_TFX_BLEND,GS_TCC_RGBA);
}
...
{
	details.ColourAdjuster.SetRGB(details.PrimColour);
	gsTexFunc(GS_TFX_DECAL,GS_TCC_RGBA);
}
...
{
	details.ColourAdjuster.SetRGB( details.EnvColour );
	gsTexFunc(GS_TFX_ADD,GS_TCC_RGB); // ADD looks better than BLEND for the sky
}
...
void BlendMode_0x00127eacf0fff238LL( BLEND_MODE_ARGS )
{
	...
	details.ColourAdjuster.SetRGB( details.EnvColour );
	gsTexFunc(GS_TFX_BLEND,GS_TCC_RGBA); //Not sure if BLEND is doing anything?
}
...
{
	details.ColourAdjuster.SetRGBA( details.EnvColour);
	gsTexFunc(GS_TFX_ADD,GS_TCC_RGB);
}
...
void BlendMode_0x00272c60350ce37fLL( BLEND_MODE_ARGS )
{
	...
	details.ColourAdjuster.SetA( details.PrimColour  );
	gsTexEnvColor( details.PrimColour.GetColour() );
	gsTexFunc(GS_TFX_BLEND,GS_TCC_RGBA);		// XXXX No T1
}
...
{
	details.ColourAdjuster.SetRGBA( details.PrimColour.ReplicateAlpha() );
	gsTexFunc(GS_TFX_REPLACE,GS_TCC_RGBA); //Wally says is REPLACE
}
