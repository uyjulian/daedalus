...
bool	PatchJumpLong( CJumpLocation jump, CCodeLabel target )
{
	...
	Ps2OpCode *	p_jump_addr( reinterpret_cast< Ps2OpCode * >( jump.GetWritableU8P() ) );
	Ps2OpCode &	op_code( *p_jump_addr );
...
{
	// Get an uncached pointer
	Ps2OpCode *	p_jump_addr( reinterpret_cast< Ps2OpCode * >( branch.GetWritableU8P() ) );
	Ps2OpCode &	op_code( *p_jump_addr );
