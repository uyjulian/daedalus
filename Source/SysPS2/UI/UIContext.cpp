...
class IUIContext : public CUIContext
{
	public:
		...
		virtual u32					GetTextWidth(const char* text, float scale) const;

	private:
				s32					AlignText( s32 min_x, s32 max_x, const char * p_str, u32 length, EAlignType align_type, float scale );
...
void	IUIContext::RenderTexture( const CNativeTexture * texture, const v2 & tl, const v2 & wh, c32 colour )
{
	...
	//u32				num_verts( 2 );
	//BackgroundTextureVtx*	p_verts = (BackgroundTextureVtx*)memalign(num_verts*sizeof(BackgroundTextureVtx));
	...
	gsKit_prim_sprite_texture_3d(gsGlobal, CurrTex,
		...
		(float)width, // U2
		(float)height, // V2
		GS_SETREG_RGBAQ(colour.GetR(), colour.GetG(), colour.GetB(), (colour.GetA()) / 2, 0x00));
}
...
u32		IUIContext::GetTextWidth( const char * text, float scale ) const
{
	return CDrawText::GetTextWidth( mCurrentFont, text, scale );
}

s32		IUIContext::AlignText( s32 min_x, s32 max_x, const char * p_str, u32 length, EAlignType align_type, float scale )
{
	s32		x;
	...
	case AT_CENTRE:
		x = min_x + ((max_x - min_x) - CDrawText::GetTextWidth( mCurrentFont, p_str, length, scale )) / 2;
		break;

	case AT_RIGHT:
		x = max_x - CDrawText::GetTextWidth( mCurrentFont, p_str, length, scale );
		break;
...
{
	s32 x( AlignText( min_x, max_x, text, length, align_type, 1.0f ) );
...
s32		IUIContext::DrawTextArea( s32 left, s32 top, u32 width, u32 height, const char * text, c32 colour, EVerticalAlign vertical_align )
{
	...
	bool				match = false;
	DrawTextUtilities::WrapText( mCurrentFont, width, CDrawText::Translate( text, length ), length, lengths, match, scale );
	...
	{
		y += font_height;
		DrawTextScale( x, y, scale, text, length, colour );
		y += 2;
		return y - top;
	}
	...
	{
		y += font_height;
		DrawTextScale( x, y, scale, text, lengths[ i ], colour );
		y += 2;
		text += lengths[ i ];
	}
