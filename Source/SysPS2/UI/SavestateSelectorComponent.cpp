...
void ISavestateSelectorComponent::LoadFolders(){
	...
	if (IO::FindFileOpen(gSaveStatesDirectory, &find_handle, find_data))
	{
		do
		{
			IO::Path::Combine( full_path, gSaveStatesDirectory, find_data.Name);
	...
	if(old_buttons != new_buttons)
	{
	  if(mAccessType == AT_LOADING && deleteButtonTriggered)
		...
		if( element != NULL )
		{
			...
			if( new_buttons & (PAD_CROSS|PAD_START) )
			{
				// Commit settings
				element->OnSelected();
			}
			if( new_buttons & PAD_SQUARE)
			{
				//delete savestate
			...
			if( new_buttons & (PAD_CIRCLE|PAD_SELECT) )
			{
				// Discard settings
				deleteButtonTriggered=false;
