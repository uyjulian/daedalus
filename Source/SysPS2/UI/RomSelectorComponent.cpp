...
void IRomSelectorComponent::Render()
{
	...
	if(mRomDelete)
	{
		mpContext->DrawTextAlign(0,630,AT_RIGHT,CATEGORY_TEXT_TOP + mpContext->GetFontHeight(),"(X) -> Confirm", color);
	}
	else
	{
		mpContext->DrawTextAlign(0,630,AT_RIGHT,CATEGORY_TEXT_TOP + mpContext->GetFontHeight(),	message[(count >> 8) % ARRAYSIZE( message )], color);
	}
...
void	IRomSelectorComponent::Update( float elapsed_time, const v2 & stick, u32 old_buttons, u32 new_buttons )
{
	...
	mDisplayFilenames = (new_buttons & PAD_TRIANGLE) != 0;

	//mDisplayInfo = (new_buttons & PAD_SQUARE) != 0;
...
#endif
		if(new_buttons & PAD_CROSS && mRomDelete)	// DONT CHANGE ORDER
		{
			remove( mSelectedRom.c_str() );
			mRomDelete = false;
			UpdateROMList();
		}
		else if((new_buttons & PAD_START) ||
			(new_buttons & PAD_CROSS))
		{
			if(mCurrentSelection < mRomsList.size())
			{
